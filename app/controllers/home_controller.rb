class HomeController < ApplicationController  
  ...
  def index
    if user_signed_in?
      ...
      # load current user's root files (uploads) (which aren't in a folder)
    ...
    # find shared folders where folder_id is equal to current folder
     ...
     if @current_folder.nil?
       ...
       @being_shared_folders = []
   	  ...
   	  @uploads = @current_folder.uploads.order("uploaded_file_file_name desc")
  ...
  def share
     ...
     email_addresses.each do |email_address|
       ...
       # getting the shared user id right the owner the email has already signed up with SecuShare
